function TrappingRainWater(arr) {
    ...
    let heightMap = buildHeighMap(arr);
...
function buildHeighMap(arr) {
    let left = arr[0], maxRight = arr[2];
    let heightMap = new Map<number, Array<number>>();
    for (let i = 1; i < arr.length - 1; i++) {
        heightMap.set(i, [left, arr[i + 1]]);

        if (arr[i - 1] > left) {
            left = arr[i - 1];
            let current = heightMap.get(i);
            current[0] = left;
...
function calculateHeight(arr, heightMap) {
    let height = 0;
    for (let i = 1; i < arr.length - 1; i++) {
        const leftRight = heightMap.get(i);
